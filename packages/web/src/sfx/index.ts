...
//
// jsdom（测试环境）没有 WebAudio：window.AudioContext 是 undefined。
// 因此这里绝不在 import 阶段构造 AudioContext，只在真正需要发声、且
// enabled 为 true 时才惰性创建；静音状态下调用任何方法都提前 return，
// 不会触碰 AudioContext。

type AudioCtor = typeof AudioContext

function resolveAudioCtor(): AudioCtor | undefined {
  if (typeof window === 'undefined') return undefined
  return (window.AudioContext ?? (window as unknown as { webkitAudioContext?: AudioCtor }).webkitAudioContext) as
    | AudioCtor
    | undefined
}
...
function env(
  node: AudioNode,
  { attack = 0.002, decay = 0.08, peak = 1 }: { attack?: number; decay?: number; peak?: number },
): GainNode {
  const c = ctx()
...
function chip(vary = 0): void {
  ...
  ;[3100 + vary * 180, 4700 + vary * 260].forEach((f, i) => {
    ...
    o.type = 'triangle'
    o.frequency.value = f
    const og = env(o, { decay: 0.035, peak: 0.1 - i * 0.04 })
    og.connect(master!)
    o.start(t)
    o.stop(t + 0.05)
  })
