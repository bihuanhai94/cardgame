...
**附加策略条款（经审查授权）：加注上限为底注的 8 倍。** 托管是在替别人花钱，保守优于聪明——一个用别人筹码乱加注的托管，比一个直接弃牌的更糟。这条同时缓解了多个 AI 互相加注的僵持，但**那不是根本修复**：引擎层允许无限加注战（人类同样可为），根因见 Task 13。真实筹码落地后需复核本条是否仍有必要，**先不要删**。
